...
class ASD_Wrapper_Visual_Encoder(nn.Module):
    """
    Wrapper around ASD visual_encoder to make it compatible with baseline AVSD framework.
    Input:  (B*num_speaker, T, 96, 96)
    Output: (B*num_speaker, T, C)  where C=128 (from ASD visual_encoder last layer)
    """
    def __init__(self):
        super(ASD_Wrapper_Visual_Encoder, self).__init__()
        self.asd_encoder = visual_encoder()  # from ASD
        self.proj = nn.Linear(128, 256)

    def forward(self, video_inputs, current_frame=None, return_embedding=True):
        """
        video_inputs: (B*num_speaker, T, H, W) = grayscale frames 96x96
        current_frame: not needed here, kept for API compatibility
        return_embedding: always returns embeddings (no logits)
        """
        BNS, T, H, W = video_inputs.shape
        # Add channel dim → (BNS, 1, T, H, W)
        x = video_inputs.unsqueeze(1)
        # Pass through ASD visual encoder
        embeddings = self.asd_encoder(x)   # (BNS, T, C)

        # Project to baseline dimension
        embeddings = self.proj(embeddings)

        if return_embedding:
            return None, embeddings   # match baseline (logits, embedding)
        else:
            return embeddings
...
if __name__ == '__main__':
    # test ASD visual encoder wrapper
    nnet = ASD_Wrapper_Visual_Encoder()

    # Dummy input: (B*num_speaker, T, 96, 96)
    # where B*num_speaker = 2, T = 20
